...
The unreleased Command Code adapter reads CLI and Desktop sessions through one Rust collector. Both clients use the visible name **Command Code**, the same Discord application, artwork and preferences.
...
The collector excludes checkpoints and hook audit logs. It deduplicates message IDs and session IDs before storage. Completed sessions remain in history. Only a recent session with active work can publish presence.
